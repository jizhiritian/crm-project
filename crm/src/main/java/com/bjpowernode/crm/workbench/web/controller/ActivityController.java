...
public class ActivityController {
    ...
    public @ResponseBody Object saveEditActivity(Activity activity,HttpSession session){
        ...
        ReturnObject returnObject=new ReturnObject();
        try {
            int ret = activityService.saveEditAvtivity(activity);
            if (ret>0){
                returnObject.setCode(Contants.RETURN_OBJECT_CODE_SUCCESS);
            }else{
                returnObject.setCode(Contants.RETURN_OBJECT_CODE_FAIL);
    ...
    public void exportAllActivitys(HttpServletResponse response) throws Exception{
       ...
       /* //根据wb对象生成excel文件
        OutputStream os=new FileOutputStream("H:\\XX\\crm-project\\xls\\activityList.xls");
        //write方法执行效率低
        wb.write(os);
        //关闭资源
        os.close();
  ...
  /*      InputStream is=new FileInputStream("H:\\XX\\crm-project\\xls\\activityList.xls");
    ...
    public @ResponseBody void exportActivitysById(String[] id,HttpServletResponse response) throws Exception{
        //调用service层方法，查询所有的市场活动
    ...
    public @ResponseBody Object importActivity(MultipartFile activityFile, String userName, HttpSession session){
        ...
        try {
            ...
            for(int i=1;i<=sheet.getLastRowNum();i++) {//sheet.getLastRowNum()：最后一行的下标    往数据库插入数据，表头不要了，从1开始
    ...
    public String detailActivity(String id,HttpServletRequest request){
        ...
        List<ActivityRemark> remarkList = activityRemarkService.queryActivityRemarkForDetailByActuvutyId(id);
        //把数据保存到作用域中
        request.setAttribute("activity",activity);
        request.setAttribute("remarkLise",remarkList);
        //请求转发
